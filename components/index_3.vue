	...
	export default {
		...
		methods: {
			...
			subMsg(){
				...
				if(this.saveUserMsgs.msg=='success')
				{
					this.$win.go('/pages/my/my?token=' +this.$db.getSync('token') +
						'&nickName=' + this.$db.getSync('nickName') + '&avatarUrl=' + this.$db.getSync('avatarUrl') )
				}
				console.log('提交信息',this.saveUserMsgs);
