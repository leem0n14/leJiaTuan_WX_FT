		...
		methods: {
			...
			async getCode() {
				...
				uni.login({
					provider: 'weixin',
					success: (loginRes) => {
						console.log(loginRes.code, "wxcode")
							...
							if (res && res.code == 0) {
								...
								getUserState({}).then((res1) => {
									...
									if (res1 && res1.code == 0) {
										...
										this.$db.setSync('identity', res1.resultMap
											.identity) //保存身份码0:路人1:学员2:教练
										if (res1.resultMap.statu == 0) {
											console.log('未登录过');
											this.$win.go('/page_login/index_2?statu=no')
										} else if (res1.resultMap.statu == 1) {
											console.log('登录过');
											if (res1.resultMap.identity == 2) {
												console.log('教练');
												this.$win.go(
													'/page_coachSection/coachSection')
											} else {
												console.log('学员');
