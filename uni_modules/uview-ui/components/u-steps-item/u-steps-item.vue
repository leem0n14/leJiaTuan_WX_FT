<template>
	<view class="u-steps-item" ref="u-steps-item" :class="[`u-steps-item--${parentData.direction}`]">
		...
		<view class="u-steps-item__wrapper"
			...
			<slot name="icon">
				...
				<view v-else :style="{
					...
					<u-icon v-else :color="statusClass === 'error' ? 'error' : parentData.activeColor" size="24rpx"
		...
		<view class="u-steps-item__content" :class="[`u-steps-item__content--${parentData.direction}`]"
			:style="[contentStyle]">
			<u--text :text="title" :type="parentData.current == index ? 'main' : 'content'" lineHeight="40rpx"
				:size="parentData.current == index ? '28rpx' : '26rpx'"></u--text>
			<slot name="desc">
				<u--text :text="desc" type="tips" size="24rpx"></u--text>
			</slot>
		</view>
<!-- 		<view
		...
		computed: {
			lineStyle() {
				...
				if (this.parentData.direction === 'row') {
					style.width = this.size.width + 'rpx'
					style.left = this.size.width / 2 + 'rpx'
				} else {
					style.height = this.size.height + 'rpx'
					// style.top = this.size.height / 2 + 'rpx'
				}
